...
def inductor_names(block: int | None = None) -> Any:
    """What code generated by TorchInductor needs to run here. Two names a CPU build of a recent
    torch does not answer: `triton_heuristics.grid` (the launch-grid helper, gone from torch since
    2.6) and `_empty_strided_cuda` (raises without CUDA); models trained on Inductor's output
    (KernelLLM, the KernelBook corpus) write both. And, for the output code of today's
    `torch.compile`, Inductor's own interpret path: `async_compile.triton` loads the kernel
    without compiling it for a device, `CachingAutotuner.run` launches through `self.fn[grid]`
    with its first config (so the session sees the launch), and the kernel source's switch to
    the GPU driver is skipped. `block` caps XBLOCK, YBLOCK and ZBLOCK of that config: a generated
    kernel is valid under every block size its autotuner may pick, and at a test's small shape
    the default one covers the whole tensor with a single program instance, in which no race
    between instances can exist. Importing Inductor costs a second or two, so this is not part
    of every session: call it when the program mentions Inductor. Returns the undo."""
    import torch
    import torch._inductor.runtime.triton_heuristics as heuristics

    guards = torch._C._dynamo.guards

    def grid(*numels: Any) -> Any:
        # as Inductor defined it: the last argument is the x dimension
        x, y, z = (list(reversed(numels)) + [None, None])[:3]

        def dim(numel: Any, block: Any) -> int:
            if numel is None:
                return 1
            return int(numel) if block is None else -(-int(numel) // int(block))

        def launch(meta: dict[str, Any]) -> tuple[int, int, int]:
            return (
                dim(x, meta.get("XBLOCK", 1)),
                dim(y, meta.get("YBLOCK")),
                dim(z, meta.get("ZBLOCK")),
            )

        return launch

    def empty_strided(size: Any, stride: Any, dtype: Any) -> Any:
        return torch.empty_strided(tuple(size), tuple(stride), dtype=dtype)

    had_grid = hasattr(heuristics, "grid")
    if not had_grid:
        heuristics.grid = grid  # type: ignore[attr-defined]
    original = guards._empty_strided_cuda
    guards._empty_strided_cuda = empty_strided

    import torch._inductor.async_compile as async_compile
    import torch._inductor.codecache as codecache
    import torch._inductor.runtime.triton_helpers as helpers

    tuner = heuristics.CachingAutotuner
    made, load, to_gpu = tuner.__init__, async_compile.AsyncCompile.triton, None

    def init(self: Any, *args: Any, **kwargs: Any) -> None:
        made(self, *args, **kwargs)
        self.triton_interpret = True  # `run` then launches `self.fn[grid]`, first config
        self._cache_eligible = False
    ...
    tuner.__init__ = init  # type: ignore[method-assign]
    async_compile.AsyncCompile.triton = triton_kernel  # type: ignore[method-assign]
    if hasattr(helpers, "set_driver_to_gpu"):
        to_gpu = helpers.set_driver_to_gpu
        helpers.set_driver_to_gpu = lambda: None  # the session has its own driver

    def undo() -> None:
        guards._empty_strided_cuda = original
        if not had_grid:
            del heuristics.grid
        tuner.__init__ = made  # type: ignore[method-assign]
        async_compile.AsyncCompile.triton = load  # type: ignore[method-assign]
        if to_gpu is not None:
            helpers.set_driver_to_gpu = to_gpu
