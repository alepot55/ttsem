...
def inductor_names() -> Any:
    """Two names that code generated by TorchInductor uses and a CPU build of a recent torch does
    not answer: `triton_heuristics.grid` (the launch-grid helper, gone from torch since 2.6) and
    `_empty_strided_cuda` (raises without CUDA). Models trained on Inductor's output (KernelLLM,
    the KernelBook corpus) write both. Importing Inductor costs a second or two, so this is not
    part of every session: call it when the program mentions them. Returns the undo."""
    import torch
